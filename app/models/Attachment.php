...
class Attachment {
    ...
    /**
     * Crea un nuevo registro de anexo en la base de datos.
     *
     * @param int $employee_id El ID del empleado al que pertenece el anexo.
     * @param string $file_name El nombre original del archivo.
     * @param string $file_path La ruta donde se guardó el archivo en el servidor.
     * @param string $attachment_type El tipo de documento (ej: Hoja de Vida).
     * @return bool Retorna true si fue exitoso, false en caso contrario.
     */
    public function create($employee_id, $file_name, $file_path, $attachment_type) {
        $query = "INSERT INTO " . $this->table_name . "
                  SET employee_id=:employee_id, file_name=:file_name, file_path=:file_path, attachment_type=:attachment_type";

        $stmt = $this->conn->prepare($query);

        // Limpiar y enlazar los parámetros
        $stmt->bindParam(":employee_id", $employee_id, PDO::PARAM_INT);
        $stmt->bindParam(":file_name", htmlspecialchars(strip_tags($file_name)));
        $stmt->bindParam(":file_path", htmlspecialchars(strip_tags($file_path)));
        $stmt->bindParam(":attachment_type", htmlspecialchars(strip_tags($attachment_type)));
